 ...
 *
 * Each drawing is the inside of a 24 × 24 viewBox and is read at 20 px on the map. The
 * lifted figures are solid and fill 21 of the 24 units; the drawn ones are stroked in
 * `currentColor`, so the colour is decided by the CSS around them.
 */
...
export const PLACE_SYMBOLS: Record<LocationKind, string> = {
  ...
  // Lifted from `docs/09-kallor/vagmarke-h5.svg`; the transform fits the
  // sign's own coordinates into the 24 × 24 box.
  mat:
  ...
  // Lifted from `docs/09-kallor/vagmarke-h28.svg`; the transform fits the
  // sign's own coordinates into the 24 × 24 box.
  husbil:
  ...
  // Lifted from `docs/09-kallor/vagmarke-h8.svg`; the transform fits the
  // sign's own coordinates into the 24 × 24 box.
  boende:
