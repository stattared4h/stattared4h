 ...
 * two sets share the stroke, the 24 × 24 viewBox and the `currentColor` fill, and cannot
 * drift apart. The animal card reuses the marker's `djurplats` figure on purpose: the
 * same animal on the nav as on the map.
 */
export const HOME_CARD_SYMBOLS: Record<HomeCardId, string> = {
  // A folded paper map, three panels with the folds drawn. Not the marker pin: a pin is
  // one place, and this card leads to all of them.
  karta:
