...
| `02-§5.54` | Överlapp bara i ytterkanten, aldrig utanför ritningen | `byggd` | `placeLabels` i `source/ts/build/map.ts` faller till det läge som överlappar minst bara när markörens prick når ritningens kant; inne på ritningen döljs det som blir över som förut. Kant och zoomhörn är hårda villkor, och en pinne kostar åtta gånger en etikett. `tests/build/map.test.ts` prövar att mitten döljer, att kant- och toppmarkörer inte gör det, att ett namn som inte ryms innanför kanten döljs ändå, och att placeringen är deterministisk |
| `02-§5.55` | Under 600 px visar överblicken inga namn | `manuell` | Mediefrågan i `source/assets/css/layout.css` döljer `.map__label` under brytpunkten om kartan varken är zoomad eller pekad på. CSS-layout går inte att enhetstesta i Node (`CL-§8.6`): öppna `/` i 360 px bredd och bekräfta att markörerna står utan namn, tryck sedan `+` tre gånger och bekräfta att namnen kommer fram; kontrollera i 1280 px att namnen syns direkt |
| `02-§5.56` | Ett streck binder etiketten till sin prick | `manuell` | `.map__label::before` i `source/assets/css/layout.css`, ett regelpar per läge — ett för de smala `--label-`-klasserna och ett för de breda `--wide-`-klasserna i mediefrågan, eftersom det är den senare som styr från 600 px. Längden räknas ur `--tap-target-min` och `--space-md` så strecket når fram i båda ändar. CSS-layout går inte att enhetstesta i Node (`CL-§8.6`): öppna `/` i 1280 px och bekräfta att varje etikett har ett streck som pekar på sin egen prick, i alla åtta lägen; zooma in i 360 px och bekräfta detsamma |
...
| `02-§5.58` | Etiketten håller sig undan pricken, inte tryckytan | `byggd` | `placeLabels` i `source/ts/build/map.ts` bygger sina spärrytor ur `LABEL_METRICS.dot` (`--space-md`) i stället för tryckytan, och `.map__label` i `source/assets/css/components.css` har `pointer-events: none` så en etikett över grannens osynliga marginal aldrig svarar på ett tryck. `tests/build/map.test.ts` kräver att måttet är tokenets och att en grannes marginal inte längre stoppar ett rakt läge |
