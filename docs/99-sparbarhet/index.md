...
| `02-§5.46` | Två djurkort i bredd på mobil, tre från desktopbrytpunkten | `byggd` | `.card-grid` i `layout.css` och kortets `sizes` i `source/layouts/animal-card.njk`; `tests/design/card-grid.test.ts` kräver båda och att de säger samma sak. Öppna `/plats/gethagen/` i 360 px: två getter i bredd |
