...
describe("the map on the home page (02-§5.23–5.27)", () => {
  test("a marker per active place with coordinates, the description, and the list", async () => {
    ...
    const markers = [...html.matchAll(/<a class="map__marker(?: map__marker--label-[\w-]+)? map__marker--wide-[\w-]+ map__marker--desktop-[\w-]+(?: map__marker--zoom-[\w-]+)?" href="\/plats\/([^/]+)\/"/g)].map((m) => m[1]);
